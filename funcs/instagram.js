    ...
    if (!get[0]) {
      ...
      if (get.length == 1) {
        if (get[0].type == 'Photo') {
          ...
          return bot.sendPhoto(chatId, get[0].thumbnail, { caption: `Powered by ISAAC ARINOLA TECH ™` })
        } else {
          try {
            await bot.sendVideo(chatId, get[0].url, { caption: `Powered by ISAAC ARINOLA TECH ™` })
          } catch (err) {
            let buff = await getBuffer(get[0].url);
            await fs.writeFileSync('content/vid-ig-single-' + chatId + '.mp4', buff)
            await bot.deleteMessage(chatId, load.message_id)
            await bot.sendVideo(chatId, 'content/vid-ig-single-' + chatId + '.mp4', { caption: `Powered by ISAAC ARINOLA TECH ™` })
        ...
        while (currentIndex < res.length) {
          ...
          if (mediaToSend.length > 0) {
            await bot.sendMediaGroup(chatId, mediaToSend, { caption: `Powered by ISAAC ARINOLA TECH ™` });
          }
        }
        ...
        res2.map(async (mi) => {
          ...
          await bot.sendVideo(chatId, 'content/' + nfile, { caption: `Powered by ISAAC ARINOLA TECH ™` })

...
async function pinSearch(bot, chatId, query, userName) {
  ...
  try {
    ...
    let json = await get.data;
		...
		await bot.sendPhoto(chatId, data[~~(Math.random() * (data.length))].images.orig.url, { caption: `Powered by ISAAC ARINOLA TECH ™` });
...
async function pinterest(bot, chatId, url, userName) {
  ...
  try {
    ...
    if (!get) {
      ...
      if (get.endsWith('.mp4')) {
        await bot.sendVideo(chatId, get, { caption: `Powered by ISAAC ARINOLA TECH ™` })
        return bot.deleteMessage(chatId, load.message_id);
      } else {
        await bot.sendPhoto(chatId, get, { caption: `Powered by ISAAC ARINOLA TECH ™` })
        return bot.deleteMessage(chatId, load.message_id);
      }
    }
  } catch (err) {

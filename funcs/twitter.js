    ...
    if (!getd.HD && !getd.SD) {
      try {
        ...
        if (get2.type == 'video') {
          ...
          await bot.sendVideo(chatId, get2.media[0].url, { caption: `Powered by 𝐈𝐒𝐀𝐀𝐂 𝐀𝐑𝐈𝐍𝐎𝐋𝐀 𝐓𝐄𝐂𝐇` })
...
async function downloadTwitterHigh(bot, chatId, userName) {
  ...
  let db = await readDb('./database.json');
  try {
    await bot.sendVideo(chatId, db[chatId].twhd, { caption: `Powered by 𝐈𝐒𝐀𝐀𝐂 𝐀𝐑𝐈𝐍𝐎𝐋𝐀 𝐓𝐄𝐂𝐇` });
    await bot.deleteMessage(chatId, load.message_id);
    db[chatId] = {
      twhd: '',
      twsd: '',
...
async function downloadTwitterLow(bot, chatId, userName) {
  ...
  let db = await readDb('./database.json');
  try {
    await bot.sendVideo(chatId, db[chatId].twsd, { caption: `Powered by 𝐈𝐒𝐀𝐀𝐂 𝐀𝐑𝐈𝐍𝐎𝐋𝐀 𝐓𝐄𝐂𝐇` });
    await bot.deleteMessage(chatId, load.message_id);
    db[chatId] = {
      twhd: '',
      twsd: '',

...
async function getYoutube(bot, chatId, url, userName) {
  ...
  try {
    if (url.includes('music.youtube.com')) {
      ...
      if (size > 49) {
        return bot.editMessageText('Boss, the file is more than 50MB, dont kill me.', { chat_id: chatId, message_id: load.message_id })
      }
...
async function getYoutubeVideo(bot, chatId, id, ind, userName) {
  ...
  try {
    ...
    let size = Math.floor(getsize.replace(' MB', ''));
    if (size > 49) {
      return bot.editMessageText('Boss, i can only download file under 50MB, dont kill me. No vex, use the link to download.\n\n' + res.downloadLink, { chat_id: chatId, message_id: load.message_id, disable_web_page_preview: true })
    }
